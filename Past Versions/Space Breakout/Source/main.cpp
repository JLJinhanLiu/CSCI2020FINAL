...
void cometStatus(unsigned int& cometY, sf::Sprite& comet,
	int& cometSpeed, int difficulty, int& heartLvl, bool invincible, sf::Sound loselife)
{
	if (cometY>800)
	{
		cometY = 0;
		comet.setPosition(rand() % 500 + 50, -100);
		cometSpeed = (rand() % 6 + difficulty);
...
void cometCollisionCheck(vector<int>& explosionClock, vector<sf::Sprite>& explosion,
	sf::FloatRect cometBox, sf::FloatRect bulletBox, int vectorIndex,
	sf::Sprite& comet, unsigned int& cometY, int& cometSpeed, int difficulty, int& bulletY, 
	sf::Sprite& bullet,
	sf::Sprite& ship, int& point, sf::Sound hitsound)
{
	if (cometBox.intersects(bulletBox))
	{
		explosionClock[vectorIndex] = 30;
		explosion[vectorIndex].setPosition(comet.getPosition());
		cometY = 0;
		comet.setPosition(rand() % 500 + 50, -100);
		cometSpeed = (rand() % 6 + difficulty);
		bulletY = 0;
...
int main()
{
	...
	while (window.isOpen()) //BEGINNING OF GAME LOOP - MENUS, GAME, ETC
	{
		...
		while (gamerunning)
		{
			...
			//setCollision
			sf::FloatRect com1box = comet1.getGlobalBounds(), com2box = comet2.getGlobalBounds(), 
				com3box = comet3.getGlobalBounds(), com4box = comet4.getGlobalBounds(); //get comet collision
			...
			//comets
			cometStatus(com1y, comet1, com1spd, diff, hearts, invincible, loselifeSound);
			cometStatus(com2y, comet2, com2spd, diff, hearts, invincible, loselifeSound);
			cometStatus(com3y, comet3, com3spd, diff, hearts, invincible, loselifeSound);
			cometStatus(com4y, comet4, com4spd, diff, hearts, invincible, loselifeSound);
			...
			cometCollisionCheck(explosion2Clock, explosion2, com1box, bulletbox,
				0, comet1, com1y, com1spd, diff, bullety, bullet, ship, point, hitSound);
			cometCollisionCheck(explosion2Clock, explosion2, com2box, bulletbox,
				1, comet2, com2y, com2spd, diff, bullety, bullet, ship, point, hitSound);
			cometCollisionCheck(explosion2Clock, explosion2, com3box, bulletbox,
				2, comet3, com3y, com3spd, diff, bullety, bullet, ship, point, hitSound);
			cometCollisionCheck(explosion2Clock, explosion2, com4box, bulletbox,
				3, comet4, com4y, com4spd, diff, bullety, bullet, ship, point, hitSound);
